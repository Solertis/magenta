...
bool rights_drop_test() {
    ...
    const uint32_t test_rights[] = { MX_RIGHT_READ, MX_RIGHT_WRITE, MX_RIGHT_EXECUTE };
    const uint32_t rwx_right = MX_RIGHT_READ | MX_RIGHT_WRITE | MX_RIGHT_EXECUTE;
    for (uint32_t right : test_rights) {
        mx_handle_t new_h;
        ASSERT_EQ(mx_handle_duplicate(vmar, right, &new_h), NO_ERROR, "");
        // Try to create a mapping with permissions we don't have
        EXPECT_EQ(mx_vmar_map(new_h, 0, vmo, 0, PAGE_SIZE, rwx_right, &map_addr),
                  ERR_ACCESS_DENIED, "");

        // Try to create a mapping with permissions we do have
        ASSERT_EQ(mx_vmar_map(new_h, 0, vmo, 0, PAGE_SIZE,
                              right, &map_addr),
                  NO_ERROR, "");

        // Attempt to use protect to increase privileges
        EXPECT_EQ(mx_vmar_protect(new_h, map_addr, PAGE_SIZE,
                                  rwx_right),
                  ERR_ACCESS_DENIED, "");
        ...
        EXPECT_EQ(mx_vmar_allocate(new_h, 0, 10 * PAGE_SIZE,
                                   rwx_right,
                                   &region, &region_addr),
                  ERR_ACCESS_DENIED, "");
...
bool nested_region_perms_test() {
    ...
    for (uint32_t perm : test_perm) {
        ...
        EXPECT_EQ(mx_vmar_allocate(region[0], 0, PAGE_SIZE, rwx_perm, &region[1], &region_addr[1]),
                  ERR_ACCESS_DENIED, "");

        // Try to create a mapping in region[0] with the dropped rights
        ASSERT_EQ(mx_vmar_map(region[0], 0, vmo, 0, PAGE_SIZE, rwx_perm, &map_addr),
                  ERR_ACCESS_DENIED, "");

        // TODO(teisenbe): Add a succesful mapping and region creation
        // underneath
